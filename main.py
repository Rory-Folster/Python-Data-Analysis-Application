...
def most_common_crimes():  # not using code comments for this function as functionality has been explained below. Have added comments to areas that contain different code.
    ...
    download_data = tk.Button(
        most_common_crimes_window, text="Download", command=save_file
    )
...
def highest_crime_areas():
    ...
    def save_file():
        file = filedialog.asksaveasfilename(
            filetypes=[("CSV Files", ".csv")], defaultextension=".csv"
        )  # remake this code from download button to upload button that will allow user to browse for csv file and either create new plot or add data to existing plot
        if file:
            data_frame.to_csv(file, index=False)
            messagebox.showinfo("CSV saved", "CSV file saved")
